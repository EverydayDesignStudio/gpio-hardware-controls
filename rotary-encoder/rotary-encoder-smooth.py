...
def init():
    ...
    GPIO.add_event_detect(Enc_A, GPIO.RISING, callback=rotary_interrupt, bouncetime=10)
    GPIO.add_event_detect(Enc_B, GPIO.RISING, callback=rotary_interrupt, bouncetime=10)
    return
...
def main():
    ...
    while True:                                 # start test
        ...
        if (NewCounter != 0):					# Counter has CHANGED
            ...
            # print('counter: {c}'.format(c=NewCounter))
            print('volume: {v}'.format(v=Volume))
